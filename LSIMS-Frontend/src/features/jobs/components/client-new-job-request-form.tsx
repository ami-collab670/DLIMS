...
import { qrTextToDataUrl } from "@/lib/media";
import {
  JOB_PRIORITY_LABEL,
...
const MAX_SAMPLES = MAX_JOB_REQUEST_SAMPLES;
...
export function ClientNewJobRequestForm({ onCreated }: Props) {
  ...
  const { mutate, isPending } = useCreateClientJobRequest({
    onSuccess: (job) => {
