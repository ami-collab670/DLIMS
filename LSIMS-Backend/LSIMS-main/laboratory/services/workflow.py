...
VALID_JOB_TRANSITIONS = {
    (JobOrder.Status.PENDING_FINANCE, JobOrder.Status.RECEIVED),
...
def code_paid_job_samples(job):
    ...
    with transaction.atomic():
        ...
        if job.current_status == JobOrder.Status.PENDING_FINANCE:
            transition_job(
                job,
                JobOrder.Status.RECEIVED,
                allowed_from=[JobOrder.Status.PENDING_FINANCE],
            )
